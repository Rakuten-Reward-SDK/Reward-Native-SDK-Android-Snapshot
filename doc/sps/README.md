...
dependencyResolutionManagement {
    repositories {
        // ...
        maven { url 'https://artifactory.rakuten-it.com/sps-android-sdk-mvn-snapshot/' }
    }
}
```  
**build.gradle**
...
RakutenMissionSps.init("platform-name") {
    val spsAudience = "rakuten_sp_scr_web"
    val artifactResponse = idSession.artifacts {
        +exchangeToken {
            audience = spsAudience
            scope = setOf(
                "sps-basic",
                "sps-point",
                "sps-register",
    ...
    artifactResponse.exchangeToken(spsAudience)?.value?.let { exchangeToken ->
